  ...
  "TARGET_68020 && TARGET_BITFIELD"
{
  if (operands[1] == const1_rtx
      && GET_CODE (operands[2]) == CONST_INT)
    {
      /* operands[0] is always MEM here (see the "memory_operand"
         predicate above), so unlike *tst_bftst_reg the bit-field
         offset (0..31, counted from the MSB) is not necessarily a
         valid single-byte btst bit number: offsets >= 8 refer to a
         byte other than the one at operands[0]'s address.  Adjust
         the address by the byte the bit actually lives in before
         normalizing the bit number to 0..7, then tell output_btst
         (via SIGNPOS 7, not 1000) that it must not try to
         byte-adjust again.  Failing to do this silently emits
         "btst #<n-24>,<original address>" for offset < 24, testing
         an unrelated bit of the wrong byte.  */
      HOST_WIDE_INT offset = INTVAL (operands[2]);
      rtx mem = adjust_address (operands[0], QImode, offset / 8);
      return output_btst (operands, GEN_INT (7 - (offset % 8)),
			  mem, insn, 7);
    }
  ...
  "TARGET_68020 && TARGET_BITFIELD"
{
  if (operands[1] == const1_rtx
      && GET_CODE (operands[2]) == CONST_INT)
    {
      /* See *tst_bftst_mem above: operands[0] is always MEM, so an
         offset >= 8 (the high byte of the 16-bit field) must shift
         the address by one byte, not just wrap the bit number.  */
      HOST_WIDE_INT offset = INTVAL (operands[2]);
      rtx mem = adjust_address (operands[0], QImode, offset / 8);
      return output_btst (operands, GEN_INT (7 - (offset % 8)),
			  mem, insn, 7);
