...
static READ16_HANDLER( fdc_r )
{
	...
	default: {
		...
		if(fdc_drq) {
			fdc_span--;
			//          loginfo(2,"Read %02x (%d)\n", res, fdc_span);
			if(fdc_span) {
				fdc_pt++;
				fdc_data = *fdc_pt;
			} else {
				loginfo(2,"FDC: transfert complete\n");
				fdc_drq = 0;
				fdc_status = 0;
				fdc_irq = 1;
			}
		} else
			loginfo(2,"FDC: data read with drq down\n");
		return res;
	}
	}
}
	...
	if(ACCESSING_LSB) {
		...
		case 0:
			...
			case 0x9:
				...
				break;
			case 0xb:
				loginfo(2,"Write multiple [%02x] %d..%d side %d track %d\n", data, fdc_sector, fdc_sector+fdc_data-1, data & 8 ? 1 : 0, fdc_phys_track);
				fdc_pt = memory_region(REGION_USER2) + track_size*(2*fdc_phys_track+(data & 8 ? 1 : 0));
				fdc_span = track_size;
				fdc_status = 3;
				fdc_drq = 1;
				break;
			case 0xd:
				loginfo(2,"FDC: Forced interrupt\n");
				fdc_span = 0;
				fdc_drq = 0;
				fdc_irq = data & 1;
				fdc_status = 0;
			...
			case 0xf:
				if(data == 0xfe)
					loginfo(2,"FDC: Assign mode %02x\n", fdc_data);
				else if(data == 0xfd)
					loginfo(2,"FDC: Assign parameter %02x\n", fdc_data);
				else
					loginfo(2,"FDC: Unknown command %02x\n", data);
				break;
			default:
				loginfo(2,"FDC: Unknown command %02x\n", data);
				break;
			...
			break;
		case 1:
			loginfo(2,"FDC: Track register %02x\n", data);
			fdc_track = data;
			break;
		case 2:
			loginfo(2,"FDC: Sector register %02x\n", data);
			fdc_sector = data;
			break;
		case 3:
			if(fdc_drq) {
				//              loginfo(2,"Write %02x (%d)\n", data, fdc_span);
				*fdc_pt++ = data;
				fdc_span--;
				if(!fdc_span) {
					loginfo(2,"FDC: transfert complete\n");
				...
				loginfo(2,"FDC: Data register %02x\n", data);
			fdc_data = data;
			break;
		}
	}
...
{
	loginfo(2,"IO daughterboard read %02x (%x)\n", offset, activecpu_get_pc());
	return 0xffff;
}

static WRITE16_HANDLER( iod_w )
{
	loginfo(2,"IO daughterboard write %02x, %04x & %04x (%x)\n", offset, data, mem_mask, activecpu_get_pc());
}
...
static WRITE16_HANDLER( mlatch_w )
{
	if(ACCESSING_LSB) {
		...
		if(!mlatch_table) {
			loginfo(2,"Protection: magic latch accessed but no table loaded (%d:%x)\n", cpu_getactivecpu(), activecpu_get_pc());
			return;
		}
		...
		if(data != 0xff) {
			...
			mlatch = data ^ mxor;
			loginfo(2,"Magic latching %02x ^ %02x as %02x (%d:%x)\n", data & 0xff, mxor, mlatch, cpu_getactivecpu(), activecpu_get_pc());
		} else {
			loginfo(2,"Magic latch reset (%d:%x)\n", cpu_getactivecpu(), activecpu_get_pc());
			mlatch = 0x00;
		}
	}
}
...
static void irq_timer_reset(void)
{
	...
	loginfo(2,"New timer frequency: %0d [%02x %04x]\n", freq, irq_timerb, irq_timera);
}
