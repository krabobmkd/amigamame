...
VIDEO_UPDATE(rbmk)
{
	...
	for (y=0;y<8;y++)
	{
		for (x=0;x<64;x++)
		{
			...
			dgp0.code = (tile&0xfff)+((tilebank&0x10)>>4)*0x1000;
			dgp0.color = tile>>12;
			dgp0.sx = x*8;
			dgp0.sy = y*32;
			drawgfx(&dgp0);
	...
	for (y=0;y<32;y++)
	{
		for (x=0;x<64;x++)
		{
			...
			dgp1.code = (tile&0xfff)+((tilebank>>1)&3)*0x1000;
			dgp1.color = tile>>12;
			dgp1.sx = x*8;
			dgp1.sy = y*8;
			drawgfx(&dgp1);
...
GAME( 1995, rbmk, 0, rbmk, rbmk,0, ROT0,  "GMS", "Real Battle Mahjong King", GAME_NOT_WORKING ,0,1,egg_Mature,0)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             