...
VIDEO_UPDATE( megazone )
{
	...
	for (offs = videoram_size - 1;offs >= 0;offs--)
	{
		if (dirtybuffer[offs])
		{
			...
			dgp0.code = ((int)videoram[offs]) + ((colorram[offs] & (1<<7) ? 256 : 0) );
			dgp0.color = (colorram[offs] & 0x0f) + 0x10;
			dgp0.flipx = flipx;
			dgp0.flipy = flipy;
			dgp0.sx = 8*sx;
			dgp0.sy = 8*sy;
	...
	/* Draw the sprites. */
	{
		...
		for (offs = spriteram_size-4; offs >= 0;offs -= 4)
		{
			...
			dgp1.code = spriteram[offs + 2];
			dgp1.color = spriteram[offs + 0] & 0x0f;
			dgp1.flipx = flipx;
			dgp1.flipy = flipy;
			dgp1.sx = sx;
			dgp1.sy = sy;
	...
	for (y = 0; y < 32;y++)
	{
		...
		for (x = 0; x < 6; x++)
		{
			...
			dgp2.code = ((int)megazone_videoram2[offs]) + ((megazone_colorram2[offs] & (1<<7) ? 256 : 0) );
			dgp2.color = (megazone_colorram2[offs] & 0x0f) + 0x10;
			dgp2.flipx = flipx;
			dgp2.flipy = flipy;
			dgp2.sx = 8*sx;
			dgp2.sy = 8*sy;
