					...
					if(tw == 0)
					{
						...
						if(t_pen)
						{
							if ( stv2_current_tilemap.colour_calculation_enabled == 0 )
								...
								((UINT16*)bitmap->line[ycnt])[xcnt+1] = alpha_blend16(((UINT16*)bitmap->line[ycnt])[xcnt+1], Machine->pens[((gfxdata[0] & 0x0f) >> 0) | (stv2_current_tilemap.bitmap_palette_number * 0x100) | pal_color_offset]);
						}
					}
					...
					if(tw == 0)
					{
						...
						if(t_pen)
						{
							if ( stv2_current_tilemap.colour_calculation_enabled == 0 )
								...
								((UINT16*)bitmap->line[ycnt])[xcnt] = alpha_blend16(((UINT16*)bitmap->line[ycnt])[xcnt], Machine->pens[((gfxdata[0] & 0x0f) >> 0) | (stv2_current_tilemap.bitmap_palette_number * 0x100) | pal_color_offset]);
						}
					}
					gfxdata++;
...
//  if (stv2_current_tilemap.layer_name==3) ui_popup ("well this is a bit  %08x %08x %08x %08x", stv2_current_tilemap.plane_size, pgtiles_x, pltiles_x, mptiles_x);
	...
	struct drawgfxParams dgp0={
		bitmap, 	// dest
		NULL, // postmoded Machine->gfx[gfx], 	// gfx
		0, 	// code
		0, 	// color
		0, 	// flipx
		0, 	// flipy
		0, 	// sx
		0, 	// sy
		&linescroll_cliprect, 	// clip
		stv2_current_tilemap.transparency, 	// transparency
		0, 	// transparent_color
		0, 	// scalex
		0, 	// scaley
		NULL, 	// pri_buffer
	...
	struct drawgfxParams dgp11={
		bitmap, 	// dest
		NULL, // Machine->gfx[gfx], // postmoded  	// gfx
		0, 	// code
		0, 	// color
		0, 	// flipx
		0, 	// flipy
	...
	for (y = 0; y<mptiles_y; y++) {
        ...
        dgp0.gfx = Machine->gfx[gfx];
        dgp11.gfx = Machine->gfx[gfx];
...
/* DRAW! */
			if(linescroll_enabled)
			{
				int start_line = y * (stv2_current_tilemap.tile_size ? 16 : 8);
				int end_line = start_line + (stv2_current_tilemap.tile_size ? 16 : 8);
				int num_lines = 0;
				INT32 scrollx;
				...
				drawypos >>= 16;
				for( ; start_line < end_line; start_line += num_lines )
				{
					num_lines = stv_vdp2_count_lines_with_equal_linescroll( start_line, end_line );
					linescroll_cliprect.min_y = -stv2_current_tilemap.scrolly + start_line;
					linescroll_cliprect.max_y = linescroll_cliprect.min_y + num_lines;
					scrollx = stv_vdp2_get_linescroll_value_for_line(start_line) >> 16;
					drawxpos = -(scrollx*scalex) + x*tilesizex*(stv2_current_tilemap.tile_size ? 2 : 1);
					drawxpos >>= 16;
					...
					if ( stv2_current_tilemap.colour_depth < 3 )
					{
						if ( stv2_current_tilemap.tile_size == 0 )
						{
							...
							if (scrollx > 0) /* wraparound x */
								
								dgp0.code = tilecode;
								dgp0.color = pal;
								dgp0.flipx = flipyx&1;
								dgp0.flipy = flipyx&2;
								dgp0.sx = drawxpos+mppixels_x;
								dgp0.sy = drawypos;
								drawgfx(&dgp0);
							if (scrollx < 0) /* wraparound x */
								...
								dgp0.flipx = !(flipyx&1);
								dgp0.flipy = flipyx&2;
								dgp0.sx = -2*scrollx-drawxpos-(stv2_current_tilemap.tile_size ? 16 : 8);
								dgp0.sy = drawypos;
								drawgfx(&dgp0);
						}
						else
						{
							...
							dgp0.code = tilecode+0+(flipyx&1)+(flipyx&2);
							dgp0.color = pal;
							dgp0.flipx = flipyx&1;
							dgp0.flipy = flipyx&2;
							dgp0.sx = drawxpos;
							...
							dgp0.code = tilecode+1-(flipyx&1)+(flipyx&2);
							dgp0.color = pal;
							dgp0.flipx = flipyx&1;
							dgp0.flipy = flipyx&2;
							dgp0.sx = drawxpos+8;
							...
							dgp0.code = tilecode+2+(flipyx&1)-(flipyx&2);
							dgp0.color = pal;
							dgp0.flipx = flipyx&1;
							dgp0.flipy = flipyx&2;
							dgp0.sx = drawxpos;
							...
							dgp0.code = tilecode+3-(flipyx&1)-(flipyx&2);
							dgp0.color = pal;
							dgp0.flipx = flipyx&1;
							dgp0.flipy = flipyx&2;
							dgp0.sx = drawxpos+8;
  ...
  };
{
								...
								dgp7.code = tilecode+0+(flipyx&1)+(flipyx&2);
								dgp7.color = pal;
								dgp7.flipx = flipyx&1;
								dgp7.flipy = flipyx&2;
								dgp7.sx = drawxpos+mppixels_x;
								dgp7.sy = drawypos;
								drawgfx(&dgp7);
								
								dgp7.code = tilecode+1-(flipyx&1)+(flipyx&2);
								dgp7.color = pal;
								dgp7.flipx = flipyx&1;
								dgp7.flipy = flipyx&2;
								dgp7.sx = drawxpos+8+mppixels_x;
								dgp7.sy = drawypos;
								drawgfx(&dgp7);
								
								dgp7.code = tilecode+2+(flipyx&1)-(flipyx&2);
								dgp7.color = pal;
								dgp7.flipx = flipyx&1;
								dgp7.flipy = flipyx&2;
								dgp7.sx = drawxpos+mppixels_x;
								...
								dgp7.code = tilecode+3-(flipyx&1)-(flipyx&2);
								dgp7.color = pal;
								dgp7.flipx = flipyx&1;
								dgp7.flipy = flipyx&2;
								dgp7.sx = drawxpos+8+mppixels_x;
					...
					{
						stv_vdp2_drawgfx_rgb555(bitmap,tilecode,flipyx&1,flipyx&2, drawxpos, drawypos,&linescroll_cliprect,stv2_current_tilemap.transparency);

						/* this isn't very efficient .. we could probably improve it */
						if (scrollx > 0) /* wraparound x */
							stv_vdp2_drawgfx_rgb555(bitmap,tilecode,flipyx&1,flipyx&2, drawxpos+mppixels_x, drawypos,&linescroll_cliprect,stv2_current_tilemap.transparency);
						if (scrollx < 0) /* wraparound x */
							stv_vdp2_drawgfx_rgb555(bitmap,tilecode,!(flipyx&1),flipyx&2, -2*scrollx-drawxpos-(stv2_current_tilemap.tile_size ? 16 : 8), drawypos,&linescroll_cliprect,stv2_current_tilemap.transparency);
			...
			else if(stv2_current_tilemap.incx != 0x10000 || stv2_current_tilemap.incy != 0x10000)
			{
			...
			else
			{
				...
				if (stv2_current_tilemap.tile_size==1)
				{
					...
					else
					{
						...
						dgp11.code = tilecode+0+(flipyx&1)+(flipyx&2);
						dgp11.color = pal;
						dgp11.flipx = flipyx&1;
						dgp11.flipy = flipyx&2;
						dgp11.sx = drawxpos;
						dgp11.sy = drawypos;
						drawgfx(&dgp11);
						
						dgp11.code = tilecode+1-(flipyx&1)+(flipyx&2);
						dgp11.color = pal;
						dgp11.flipx = flipyx&1;
						dgp11.flipy = flipyx&2;
						dgp11.sx = drawxpos+8;
						...
						dgp11.code = tilecode+2+(flipyx&1)-(flipyx&2);
						dgp11.color = pal;
						dgp11.flipx = flipyx&1;
						dgp11.flipy = flipyx&2;
						dgp11.sx = drawxpos;
						...
						dgp11.code = tilecode+3-(flipyx&1)-(flipyx&2);
						dgp11.color = pal;
						dgp11.flipx = flipyx&1;
						dgp11.flipy = flipyx&2;
						dgp11.sx = drawxpos+8;
...
static void stv_vdp2_draw_rotation_screen(mame_bitmap *bitmap, const rectangle *cliprect, int iRP)
{
	...
	else
	{
		...
		{
			stv_vdp2_roz_bitmap[iRP-1] = auto_bitmap_alloc( 4096, 4096 );
		}
