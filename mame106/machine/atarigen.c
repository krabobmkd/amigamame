...
{
	mame_timer_adjust(scanline_interrupt_timer, cpu_getscanlinetime_mt(scanline), 0, time_zero);
}
...
void atarigen_scanline_timer_reset(atarigen_scanline_callback update_graphics, int frequency)
{
	...
	mame_timer_set(cpu_getscanlinetime_mt(Machine->drv->screen_height), 0, vblank_timer);
}
...
static void vblank_timer(int param)
{
	...
	/* set a timer to go off on the next VBLANK */
	mame_timer_set(cpu_getscanlinetime_mt(Machine->drv->screen_height), 1, vblank_timer);
}
...
static void atarivc_eof_update(int param)
{
	...
	mame_timer_set(cpu_getscanlinetime_mt(0), 0, atarivc_eof_update);
}
