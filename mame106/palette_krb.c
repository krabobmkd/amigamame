...
//krb
static pen_t blue_pen, grey_pen;
...
static int palette_alloc(void)
{
	int max_total_colors = total_colors + 4;
	int i;
	...
	switch (colormode)
	{
		...
		case PALETTIZED_16BIT:
		{
			...
			if (total_colors_with_ui <= 65534)
			{
                ...
                game_palette[total_colors + 2] = adjusted_palette[total_colors + 2] = MAKE_RGB(0x10,0x10,0x60);
				game_palette[total_colors + 3] = adjusted_palette[total_colors + 3] = MAKE_RGB(0xaa,0xaa,0xaa);
