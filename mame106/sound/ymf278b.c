...
// krb modified because GCC6.5 would generate bad FPU instruction at some point (???)
// -> GCC make it crash with O3 ->O2 good enough
...
static void ymf278b_envelope_next(YMF278BSlot *slot, float clock_ratio)
{
	...
	if(slot->env_step == 2)
	{
		...
		else
        {
		 ...
		 UINT32 divc =  (UINT32)(ymf278_compute_decay_rate(rate) * clock_ratio);
		 if(divc ==0) divc=1;
			slot->env_vol_step = ((256U-slot->DL*8)<<23) / divc ;
        }
		slot->env_vol_lim = 256U<<23;
		slot->env_step++;
		return;
 ...
 //printf("in mix\n");
			for (j = 0; j < length; j++)
			{
				...
				*mixp++ += (sample * chip->volume[slot->TL+chip->pan_right[slot->pan]+(slot->env_vol>>23)])>>17;
				...
				{
// printf("ymf278b_envelope_next c1\n");
...
// printf("ymf278b_envelope_next c2\n");
					ymf278b_envelope_next(slot, chip->clock_ratio);
				...
				else
				{
					...
					{
						slot->env_step = 4;
// printf("ymf278b_envelope_next c3\n");
						ymf278b_envelope_next(slot, chip->clock_ratio);
					}
				}
				break;
...
static void ymf278b_init(YMF278BChip *chip, UINT8 *rom, void (*cb)(int), int clock)
{
	...
	chip->clock_ratio = (float)clock * (1.0f/(float)YMF278B_STD_CLOCK);
}
...
static void *ymf278b_start(int sndindex, int clock, const void *config)
{
	...
	for(i = 0; i < 256; i++)
		chip->volume[i] = 65536*powf(2.0f, (-0.375f/6.0f)*i);
