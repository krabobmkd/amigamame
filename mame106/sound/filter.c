...
filter* filter_lp_fir_alloc(float freq, int order) {
	...
	for(i=1;i<=midorder;++i) {
		...
		/* sample value */
        float c = 1.0f; //sinf((float)(2.0f*M_PI)*freq*i) / (M_PI*i);

		/* apply only one window or none */
		/* float w = 2 - 2*n/(order-1); */ /* Bartlett (triangular) */
		/* float w = 0.5 * (1 - cos(2*M_PI*n/(order-1))); */ /* Hanning */
		float w = 1.0f;// 0.54f - (0.46f * cosf((float)(2.0f*(float)M_PI)*n)
                        //   /(float)(order-1)); /* Hamming */
	...
	for(i=0;i<=midorder;++i) {
		...
		f->xcoeffs[i] /= gain;
#else
		f->xcoeffs[i] = f->xcoeffs[i] * (float)(1 << FILTER_INT_FRAC) / gain;
#endif
	}

	/* decrease the order if the last coeffs are 0 */
	i = midorder;
	while (i > 0 && f->xcoeffs[i] == 0.0f)
		--i;
...
void filter2_setup(int type, float fc, float d, float gain,
					struct filter2_context *filter)
{
	float w;	/* cutoff freq, in radians/sec */
	float w_squared;
	float den;	/* temp variable */
	float two_over_T = 2*Machine->sample_rate;
	float two_over_T_squared = two_over_T * two_over_T;
	...
	filter->a1 = 2.0f*(-two_over_T_squared + w_squared)/den;
...
void filter_opamp_m_bandpass_setup(float r1, float r2, float r3, float c1, float c2,
					struct filter2_context *filter)
{
	float	r_in, fc, d, gain;
	...
	{
		gain = r2 / (r1 + r2);
		r_in = 1.0f / (1.0f/r1 + 1.0f/r2);
	}

	fc = 1.0f / (2 * M_PI * sqrt(r_in * r3 * c1 * c2));
