...
static YM2151 * PSG;
// krb: this is kept because lfo operator use output.
...
PSG->statemask|=opmask; (op)->volume += (~(op)->volume *					\
...
INLINE void envelope_KONKOFF(UINT32 opmask, YM2151Operator * op, int v)
{
	if (v&0x08)	/* M1 */
		KEY_ON (op+0, 1)
	else
...
INLINE void refresh_EG(YM2151Operator * op)
{
    ...
    for(UINT16 i=0;i<4 ;i++)
    {
        ...
        if ((op->ar+v) < 32+62)
        {
            ...
            op->eg_sh_ar_mask = (1UL<<op->eg_sh_ar)-1;
...
void YM2151WriteReg(void *_chip REGYM(a0), UINT8 r REGYM(d0), UINT8 v  REGYM(d1))
{
	YM2151 *chip = _chip;
...
//krb	v &= 0xff;
	...
	case 0x00:
        ...
        {
            UINT8 iop = (v&7)<<2;
			envelope_KONKOFF(1UL<<iop, &chip->oper[iop], v );
        }
			break;
...
INLINE void advance_eg(void)
{
    ...
    if(PSG->eg_timer >= PSG->eg_timer_overflow)
	{
        ...
        UINT32 irevmask =1;
		for(UINT32 statemask = PSG->statemask;
            statemask;
            (statemask>>=1))
		{
			switch(op->state)
			...
			case EG_ATT:	/* attack phase */
				if ( !(PSG->eg_cnt & op->eg_sh_ar_mask) )
				{
					op->volume += (~op->volume *
                                   (eg_inc[op->eg_sel_ar + ((PSG->eg_cnt>>op->eg_sh_ar)&7)])
                                  ) >>4;
			...
			case EG_DEC:	/* decay phase */
				if ( !(PSG->eg_cnt & op->eg_sh_d1r_mask) )
				{
			...
			case EG_SUS:	/* sustain phase */
				if ( !(PSG->eg_cnt & op->eg_sh_d2r_mask) )
				{
			...
			case EG_REL:	/* release phase */
				if ( !(PSG->eg_cnt & op->eg_sh_rr_mask) )
				{
...
void YM2151UpdateOne(void *chip, SAMP **buffers, int length)
{
	...
	for (i=0; i<length; i++)
	{
        ...
        // krb moved to loop, better for cache
...
// krb: this is done once on final mixing.
