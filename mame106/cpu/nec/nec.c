...
OP( 0x0f, i_pre_nec  ) { UINT32 ModRM, tmp, tmp2;
	switch (FETCH) {
		...
		case 0x31 : ModRM = FETCH; ModRM=0; loginfo(2,"%06x: Unimplemented bitfield INS\n",activecpu_get_pc()); break;
		case 0x33 : ModRM = FETCH; ModRM=0; loginfo(2,"%06x: Unimplemented bitfield EXT\n",activecpu_get_pc()); break;
		case 0x92 : CLK(2); break; /* V25/35 FINT */
		case 0xe0 : ModRM = FETCH; ModRM=0; loginfo(2,"%06x: V33 unimplemented BRKXA (break to expansion address)\n",activecpu_get_pc()); break;
		case 0xf0 : ModRM = FETCH; ModRM=0; loginfo(2,"%06x: V33 unimplemented RETXA (return from expansion address)\n",activecpu_get_pc()); break;
		case 0xff : ModRM = FETCH; ModRM=0; loginfo(2,"%06x: unimplemented BRKEM (break to 8080 emulation mode)\n",activecpu_get_pc()); break;
		default:    loginfo(2,"%06x: Unknown V20 instruction\n",activecpu_get_pc()); break;
	}
}
...
OP( 0x62, i_chkind  ) {
    ...
    if (tmp<low || tmp>high) {
 	...
 	nec_ICount-=20;
	loginfo(2,"%06x: bound %04x high %04x low %04x tmp\n",activecpu_get_pc(),high,low,tmp);
}
...
OP( 0x65, i_repc  ) { 	UINT32 next = FETCHOP;	UINT16 c = I.regs.w[CW];
    ...
    switch(next) {
	    ...
	    case 0xae:	CLK(2); if (c) do { i_scasb(); c--; } while (c>0 && CF);	I.regs.w[CW]=c; break;
	    case 0xaf:	CLK(2); if (c) do { i_scasw(); c--; } while (c>0 && CF);	I.regs.w[CW]=c; break;
		default:	loginfo(2,"%06x: REPC invalid\n",activecpu_get_pc());	nec_instruction[next]();
    }
	seg_prefix=FALSE;
}
...
OP( 0x8e, i_mov_sregw ) { UINT16 src; GetModRM; src = GetRMWord(ModRM); CLKR(15,15,7,15,11,5,2,EA);
    switch (ModRM & 0x38) {
		...
		default:   loginfo(2,"%06x: Mov Sreg - Invalid register\n",activecpu_get_pc());
    }
	I.no_interrupt=1;
}
...
OP( 0x9b, i_wait      ) { loginfo(2,"%06x: Hardware POLL\n",activecpu_get_pc()); }
#ifdef _MSC_VER
#pragma optimize("",off)
#endif
...
OP( 0xc0, i_rotshft_bd8 ) {
	...
	if (c) switch (ModRM & 0x38) {
		...
		case 0x30: loginfo(2,"%06x: Undefined opcode 0xc0 0x30 (SHLA)\n",activecpu_get_pc()); break;
...
OP( 0xc1, i_rotshft_wd8 ) {
    ...
    if (c) switch (ModRM & 0x38) {
		...
		case 0x30: loginfo(2,"%06x: Undefined opcode 0xc1 0x30 (SHLA)\n",activecpu_get_pc()); break;
...
OP( 0xd0, i_rotshft_b ) {
    ...
    switch (ModRM & 0x38) {
		...
		case 0x30: loginfo(2,"%06x: Undefined opcode 0xd0 0x30 (SHLA)\n",activecpu_get_pc()); break;
...
OP( 0xd1, i_rotshft_w ) {
	...
	switch (ModRM & 0x38) {
		...
		case 0x30: loginfo(2,"%06x: Undefined opcode 0xd1 0x30 (SHLA)\n",activecpu_get_pc()); break;
...
OP( 0xd2, i_rotshft_bcl ) {
	...
	if (c) switch (ModRM & 0x38) {
		...
		case 0x20: SHL_BYTE(c); break;
		case 0x28: SHR_BYTE(c); break;
		case 0x30: loginfo(2,"%06x: Undefined opcode 0xd2 0x30 (SHLA)\n",activecpu_get_pc()); break;
		case 0x38: SHRA_BYTE(c); break;
	}
}
...
OP( 0xd3, i_rotshft_wcl ) {
    ...
    if (c) switch (ModRM & 0x38) {
		...
		case 0x20: SHL_WORD(c); break;
		case 0x28: SHR_WORD(c); break;
		case 0x30: loginfo(2,"%06x: Undefined opcode 0xd3 0x30 (SHLA)\n",activecpu_get_pc()); break;
		case 0x38: SHRA_WORD(c); break;
	}
}
...
OP( 0xd6, i_setalc ) { I.regs.b[AL] = (CF)?0xff:0x00; nec_ICount-=3; loginfo(2,"%06x: Undefined opcode (SETALC)\n",activecpu_get_pc()); }
OP( 0xd7, i_trans  ) { UINT32 dest = (I.regs.w[BW]+I.regs.b[AL])&0xffff; I.regs.b[AL] = GetMemB(DS, dest); CLKS(9,9,5); }
OP( 0xd8, i_fpo    ) { GetModRM; nec_ICount-=2;	loginfo(2,"%06x: Unimplemented floating point control %04x\n",activecpu_get_pc(),ModRM); }
...
OP( 0xf0, i_lock     ) { loginfo(2,"%06x: Warning - BUSLOCK\n",activecpu_get_pc()); I.no_interrupt=1; CLK(2); }
OP( 0xf2, i_repne    ) { UINT32 next = FETCHOP; UINT16 c = I.regs.w[CW];
    ...
    switch(next) {
		...
		default:	loginfo(2,"%06x: REPNE invalid\n",activecpu_get_pc());	nec_instruction[next]();
    }
	seg_prefix=FALSE;
}
OP( 0xf3, i_repe     ) { UINT32 next = FETCHOP; UINT16 c = I.regs.w[CW];
    ...
    switch(next) {
		...
		default:	loginfo(2,"%06x: REPE invalid\n",activecpu_get_pc()); nec_instruction[next]();
    }
	seg_prefix=FALSE;
}
OP( 0xf4, i_hlt ) { loginfo(2,"%06x: HALT\n",activecpu_get_pc()); nec_ICount=0; }
...
OP( 0xf6, i_f6pre ) { UINT32 tmp; UINT32 uresult,uresult2; INT32 result,result2;
    ...
    switch (ModRM & 0x38) {
		...
		case 0x08: loginfo(2,"%06x: Undefined opcode 0xf6 0x08\n",activecpu_get_pc()); break;
...
OP( 0xf7, i_f7pre   ) { UINT32 tmp,tmp2; UINT32 uresult,uresult2; INT32 result,result2;
    ...
    switch (ModRM & 0x38) {
		...
		case 0x08: loginfo(2,"%06x: Undefined opcode 0xf7 0x08\n",activecpu_get_pc()); break;
...
OP( 0xfe, i_fepre ) { UINT32 tmp, tmp1; GetModRM; tmp=GetRMByte(ModRM);
    switch(ModRM & 0x38) {
		...
		default:   loginfo(2,"%06x: FE Pre with unimplemented mod\n",activecpu_get_pc());
	}
}
OP( 0xff, i_ffpre ) { UINT32 tmp, tmp1; GetModRM; tmp=GetRMWord(ModRM);
    switch(ModRM & 0x38) {
		...
		default:   loginfo(2,"%06x: FF Pre with unimplemented mod\n",activecpu_get_pc());
	}
}
