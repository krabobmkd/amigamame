...
FILE *traceFH = NULL;
UINT64 nbt=0;
UINT64 mintrace=  0x00000000086fb587; // 0x0fffffffffffffff ;
// min 0x00000000086fb587
// max 0x00000000092f339c
UINT64 nbtt=0;
...
void openTraceFH()
{
    nbtt++;
    nbt++;
    if(nbt>=0x00000000092f339c) exit(1);
    if(traceFH)
    {
        if(nbtt==32) {
        nbtt = 0;
    ...
    traceFH = fopen("memtraceO.txt","wb");
}
...
#if DOTRACEMEM
    m68k_memory_intf =   m68k_memory_tracer_d16;
#else
    m68k_memory_intf =   interface_d16;
#endif
...
#if DOTRACEMEM
    m68k_memory_intf =   interface_tracer_d32;
#else
    m68k_memory_intf =   interface_d32;
#endif
