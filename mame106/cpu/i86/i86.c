...
static int i86_execute(int num_cycles)
{
    ...
    //krb says:
    // better this here
    // All pre-i286 CPUs have a 1MB address space  (and erset to 0f fff0) ,
    // but qbert has a ffff memory entry for code, so ...
    // the mask test were removed from common acessor for optimisation.
    I.pc &= opcode_mask;
