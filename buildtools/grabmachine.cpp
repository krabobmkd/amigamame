...
static const char *machinename="ezer";
struct woot {
    const char *m;
    int h;
};
...
bool changeapi(std::string ofilepath,std::string nfilepath,std::string basename)
{
...
//    ofsb << "static const char *machinename=\"" << basename << "\";\n";
    ofsb << "#define MACHINENAME \"" << basename << "\"\n";
...
//            break;
//    }
    bfile.substros(ofsb,0,bfile.length()-0);
...
int main(int argc, char **argv)
{
   ...
   for (const auto & entry : fs::directory_iterator(sdir))
   {
      ...
      sname.rfind(".h") != sname.length()-2
        ...
        string basename = sname.substr(0,sname.length()-2);
        changeapi(ofilepath,nfilepath,basename);
   }
